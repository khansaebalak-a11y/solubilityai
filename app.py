...
/* ── Nav boutons mobile (actif / inactif) ── */
.nav-btn-active .stButton > button {
    background: rgba(56,178,172,.18) !important;
    color: #38b2ac !important;
    border: 1px solid rgba(56,178,172,.4) !important;
...
.nav-btn-inactive .stButton > button {
...
.nav-btn-inactive .stButton > button:hover {
...
# ══════════════════════════════════════════════════════════════════
# SIDEBAR (PC uniquement — kept for wide screens)
# ══════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown(f"""
...
# ══════════════════════════════════════════════════════════════════
# NAVIGATION MOBILE — boutons Streamlit natifs en haut de page
# ══════════════════════════════════════════════════════════════════
def render_mobile_nav():
    """Barre de navigation avec boutons Streamlit natifs — fonctionne sur mobile."""
    current = st.session_state.page

    # Header logo
    st.markdown(f"""
    <div style="display:flex;align-items:center;justify-content:space-between;
        padding:10px 0 12px;border-bottom:1px solid rgba(255,255,255,.08);margin-bottom:10px;">
      <div style="display:flex;align-items:center;gap:10px;">
        <div style="background:linear-gradient(135deg,#38b2ac,#4299e1);
            width:32px;height:32px;border-radius:8px;display:flex;align-items:center;
            justify-content:center;font-size:16px;">🧪</div>
        <div>
          <div style="color:white;font-size:13px;font-weight:700;line-height:1.1;">SolubilityAI</div>
          <div style="color:#38b2ac;font-size:9px;">{BEST_NAME} + SHAP</div>
        </div>
      </div>
      <div style="background:linear-gradient(135deg,#38b2ac,#4299e1);color:white;
    ...
    # Boutons natifs Streamlit en colonnes — marchent partout
    cols = st.columns(5)
    for col, (pid, icon, label) in zip(cols, NAV):
        with col:
            is_active = (pid == current)
            btn_style = "nav-btn-active" if is_active else "nav-btn-inactive"
            st.markdown(f'<div class="{btn_style}">', unsafe_allow_html=True)
            if st.button(f"{icon}\n{label}", key=f"mnav_{pid}", use_container_width=True):
                st.session_state.page = pid
                st.session_state.analyse_result = None
                st.rerun()
